...
void ftrce_text_jump(word_t pc){
    ...
    Log_func(ANSI_FMT("func trace", ANSI_FG_BLUE)" "FMT_WORD ": \t", pc);
    for (int i = 0; i < func;i++){
        Log_func("\t");
    }
    Log_func(ANSI_FMT("call",ANSI_FG_GREEN)" [");
    Log_func(ANSI_FMT("%s@"FMT_WORD,ANSI_FG_YELLOW), name,first_addr);
    Log_func("]\n");
    func++;
    return;
}
...
void symbol_list_push(symbol_list *list,char *name,word_t first_addr,word_t func_size){
    ...
    // rv64g 允许压缩指令(2字节), 函数最后一条指令可能是2字节, 所以用-2而不是-4
    new->end_addr=first_addr+func_size-2;
    new->prev=NULL;
    new->next=list->head;
    list->head=new;
    list->node_num++;
...
void init_ftrace(const char *ELF_FILE){
    ...
    assert(elf_file);
    Elf64_Ehdr text;
    a=fread(&text,1,sizeof(Elf64_Ehdr),elf_file);
    assert(a==sizeof(Elf64_Ehdr));
    ...
    Elf64_Shdr symbol_tab = {.sh_offset = 0, .sh_size = 0};
    Elf64_Shdr str_tab = {.sh_offset = 0, .sh_size = 0};
    ...
    for(int i=0;i<symbol_tab.sh_size/sizeof(Elf64_Sym);i++){
        Elf64_Sym symbol;
        fseek(elf_file,symbol_tab.sh_offset+sizeof(Elf64_Sym)*i,SEEK_SET);
        a=fread(&symbol,sizeof(Elf64_Sym),1,elf_file);
        assert(a == 1);
